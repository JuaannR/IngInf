	...
	private void sendResponse(DatagramPacket pkt) throws IOException {
		...
		case DirMessageOps.OPERATION_SERVE: {
			...
			if (dmDatos.getFileList() == null) {
			    ...
			    dmRespuesta = new DirMessage(DirMessageOps.OPEARTION_SERVER_ERROR); // si lo tienes, o SERVEOK si quieres forzar
			    break;
			}
		...
		case DirMessageOps.OPERATION_DOWNLOAD: {
			String requestedSubstring = dmDatos.getFileNameSubstring();
			ArrayList<InetSocketAddress> matchingServers = new ArrayList<>();
			...
			for (Map.Entry<InetSocketAddress, ArrayList<FileInfo>> entry : serverToFileList.entrySet()) {
				for (FileInfo f : entry.getValue()) {
					if (f.fileName.contains(requestedSubstring)) {
						matchingServers.add(entry.getKey());
						break; // Ya sabemos que este servidor lo tiene
					}
				}
			}

...
void Interprete::AND()
{
    ...
    // recuperamos las referencias de cada pagina, y creamos un array con iteradores
    int nPal = palabras.size();
    list<Pagina*> tabla_referencias[nPal]; //lista de listas de referencias
    std::list<Pagina*>::iterator its[nPal]; //lista de iteradores
    std::list<Pagina*>::iterator ends[nPal]; //lista de ends
...
int hayElem = true; //para salir cuando no queden más
    list<Pagina*> referencias;
    while (hayElem) {
        // Buscar el próximo elemento más entre las listas
        ...
        for (int i = 0; i < nPal; ++i) {
            if (its[i] != ends[i]) {
                ...
                if (mejorPagina == NULL || 
                    ...
                    (actual->getRelev() == mejorPagina->getRelev() && actual->getURL() < mejorPagina->getURL())) {
                    mejorPagina = actual;
                }
            }
        }
        // Si no se encontró ningún mejor elemento, `hayElementos` será falso
        if (mejorPagina) {
